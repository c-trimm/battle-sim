...
define(['module/engine', 'module/tactics', 'module/map'], function(engine, tacticFunctions, map) {
    return function(troopData) {
        ...
        troop.moveWithinRange = function(path) {
            ...
            var newPath = path.slice(0, $speed);
            ...
            if ((lastNodeInMovementRange.y == targetNode.y) && (lastNodeInMovementRange.x == targetNode.x)) {
                ...
                if (newPath.length > 1) newPath = newPath.slice(0, newPath.length - this.range); // get the closest node within targeting range of target
